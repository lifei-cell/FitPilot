...
### P1：产品闭环与 AI 体验（已实现，待真实 Compose 复验）

#### 4.4 Agent 长期会话与跨设备恢复

- 已由 V12 建立 PostgreSQL 真源、Redis 最近 30 条/2 小时热缓存、历史分页与 180 天保留任务。
- 已提供会话列表、重命名、归档、删除、跨设备历史和一次性确认令牌重新签发。

验收标准：跨浏览器登录可恢复历史；Redis 清空后仍可读取已归档会话；删除和数据保留策略有集成测试。

#### 4.5 基于训练反馈的计划调节闭环

- V13 已采集 Workout 完成率、工作组 RPE、疼痛/疲劳、容量和 PR 趋势，并输出 28 天指标快照。
- 确定性规则约束 LLM 差异，动作替换要求同主肌群和有效引用；确认只创建新 DRAFT，ACTIVE 不变。
- 调整原因、证据、计划版本、模型、Prompt、确认/拒绝/失效状态均可审计。

验收标准：建议可解释、可拒绝、可回滚；越界负荷、伤病风险词和缺失数据均有安全策略；离线评测覆盖正常、边界和对抗样本。

#### 4.6 RAG 内容治理与反馈闭环

- V14 已增加发布方、固定可信等级、有效期、不可变版本链、恢复新版本与删除传播任务。
- 回答/Citation 反馈按用户 Upsert，人工审核和正确来源进入版本化动态评测集，不直接干预在线排序。
- 评测冻结静态与动态样本，按类别输出 Recall@5/MRR；Citation Validity 100% 与分类 5pp 回归为硬门禁。
