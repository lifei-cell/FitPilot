...
public class EvaluationRepository {
    ...
    public void createRag(UUID id,String dataset,Map<String,Object> snapshot){jdbc.update("INSERT INTO rag_eval_run(id,dataset_version,dataset_snapshot,status,started_at) VALUES (?,?,?::jsonb,'RUNNING',now())",id,dataset,write(snapshot));}
    ...
    public void ragResult(UUID run,String caseId,String hash,List<String> expectedSources,List<String> actualSources,
                          ...
                          boolean citationValid,long latency){jdbc.update("INSERT INTO rag_eval_result(run_id,case_id,query_hash,expected_source_urls,actual_source_urls,recall_at_5,reciprocal_rank,ndcg,context_precision,context_recall,citation_valid,latency_ms) VALUES (?,?,?,?::jsonb,?::jsonb,?,?,?,?,?,?,?)",run,caseId,hash,write(expectedSources),write(actualSources),recall,rr,ndcg,precision,contextRecall,citationValid,latency);}
    ...
    public void failRagGate(UUID id,int total,int passed,Map<String,Double> metrics,String message){jdbc.update("UPDATE rag_eval_run SET status='FAILED',total_cases=?,passed_cases=?,metrics=?::jsonb,error_message=?,completed_at=now() WHERE id=?",total,passed,write(metrics),safe(message),id);}
    public Map<String,Double> lastSuccessfulRagMetrics(UUID excluding){return jdbc.query("SELECT metrics::text FROM rag_eval_run WHERE status='SUCCEEDED' AND id<>? ORDER BY completed_at DESC LIMIT 1",rs->rs.next()?readMetrics(rs.getString(1)):Map.of(),excluding);}
