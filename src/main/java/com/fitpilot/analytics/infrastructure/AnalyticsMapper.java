...
public interface AnalyticsMapper {
    @Select("SELECT COUNT(*) FROM workout_analytics_projection WHERE user_id=#{userId} " +
            "AND completed_at >= #{start} AND completed_at < #{end}")
    long workoutCount(long userId, LocalDateTime start, LocalDateTime end);

    @Select("SELECT COALESCE(SUM(duration_seconds), 0) FROM workout_analytics_projection WHERE user_id=#{userId} " +
            "AND completed_at >= #{start} AND completed_at < #{end}")
    long trainingDurationSeconds(long userId, LocalDateTime start, LocalDateTime end);

    @Select("SELECT COALESCE(SUM(training_volume), 0) FROM workout_analytics_projection " +
            "WHERE user_id=#{userId} AND completed_at >= #{start} AND completed_at < #{end}")
